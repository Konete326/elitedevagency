...
import { Printer, RefreshCw, Save, Edit2, Trash2, ToggleLeft, ToggleRight, FileText, ArrowLeft, Plus } from 'lucide-react';
        ...
        {activeTab === 'wallet' && user?.niche === 'RESTAURANT' && (
          <div className="w-full">
            {(action === 'add' || action === 'edit') ? (
                ...
                <form onSubmit={handleFormSubmit} className="space-y-3.5">
                  ...
                  <div className="space-y-1">
                    ...
                    {getFieldStatus('accountTitle', accountTitle) === 'invalid' && (
                      <p className="text-[9px] text-red-500 font-bold">Title contains letters and spaces only</p>
                    )}
                  </div>
                  ...
                  {methodType === 'BANK' && (
                    <div className="space-y-1">
                      ...
                      {getFieldStatus('iban', iban) === 'invalid' && (
                        <p className="text-[9px] text-red-500 font-bold font-mono">Must be a valid alphanumeric code (mandatory if Account number is missing)</p>
                      )}
                    </div>
                  )}
              ...
              <div className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-4 w-full">
                <div className="flex items-center justify-between border-b border-border pb-3">
                  <div>
                    <h2 className="text-sm font-bold tracking-tight">Configured Payment Gateways</h2>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 bg-primary text-primary-foreground hover:opacity-95 font-bold rounded-lg px-3.5 py-2 text-xs transition-all cursor-pointer"
                  >
                ...
                {paymentMethods.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
                    {paymentMethods.map((item) => (
                      <div key={item.id} className="p-3.5 rounded-xl border border-border bg-muted/20 flex flex-col justify-between space-y-3 relative">
                        ...
                        <div className="flex items-center justify-end gap-1.5 border-t border-border/60 pt-2">
                          <button
                            ...
                            className="p-1 rounded border border-border bg-card hover:bg-muted text-muted-foreground transition-colors cursor-pointer"
                          >
                            <Edit2 className="h-3 w-3" />
                          </button>
                          <button
                            onClick={() => handleDelete(item.id)}
                            className="p-1 rounded border border-red-500/10 bg-card hover:bg-red-500/5 text-red-500 transition-colors cursor-pointer"
                          >
